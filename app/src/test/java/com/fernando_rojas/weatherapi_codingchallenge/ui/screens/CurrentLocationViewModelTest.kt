...
import com.fernando_rojas.weatherapi_codingchallenge.data.model.geocoding.LocalNamesModel
import com.fernando_rojas.weatherapi_codingchallenge.data.model.weather.CloudingModel
import com.fernando_rojas.weatherapi_codingchallenge.data.model.weather.CoordModel
import com.fernando_rojas.weatherapi_codingchallenge.data.model.weather.MainModel
import com.fernando_rojas.weatherapi_codingchallenge.data.model.weather.SysModel
import com.fernando_rojas.weatherapi_codingchallenge.data.model.weather.WeatherModel
import com.fernando_rojas.weatherapi_codingchallenge.data.model.weather.WindyModel
import com.fernando_rojas.weatherapi_codingchallenge.data.repository.Repository
...
class CurrentLocationViewModelTest {
    ...
    fun getWeather_shouldUpdateWeatherState() = runBlocking {
        ...
        val units = "metric"
        val apiKey = "API_KEY"
        val expectedWeather = WeatherModel(base = "", clouds = CloudingModel(), cod = 0, coord = CoordModel(), dt = 0, id = 0, main = MainModel(), name = "", sys = SysModel(), timezone = 0, visibility = 0, weather = listOf(), wind = WindyModel())
        val expectedForecast = ForecastModel( cnt = 0, code = "", list = listOf(), message = 0)
    ...
    fun getLocation_shouldUpdateSearchState() = runBlocking {
        ...
        val expectedResponse = listOf(
            GeocodingModel(cntry = "", latitude = 0.0, localNames = LocalNamesModel(), longitude = 0.0, name = "", state = ""),
            GeocodingModel(cntry = "", latitude = 0.0, localNames = LocalNamesModel(), longitude = 0.0, name = "", state = ""),
            GeocodingModel(cntry = "", latitude = 0.0, localNames = LocalNamesModel(), longitude = 0.0, name = "", state = "")
        )
